...
class ProfileRosaOnlineBaselineSmokeTests(unittest.TestCase):
    def test_run_profile_writes_report_and_keeps_online_reference_aligned(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertIn("train_path_consistency", report)
            ...
            self.assertTrue(report["train_path_consistency"]["enabled"])
            self.assertEqual(report["train_path_consistency"]["correctness"]["address_agreement"]["all_equal"], 1.0)
            ...
            self.assertLessEqual(report["summary"]["train_path_logit_max_abs_diff"], 1e-6)
